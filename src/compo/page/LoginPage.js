...
const LoginPage = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const portalID = 1;
      const response = await axios.post(loginapi, { username: email, password, portalID }, { withCredentials: true });
      if (response.status === 200) {
        if (response.data.firstLogin) {
          Swal.fire('Welcome!', 'Please change your password.', 'info');
          localStorage.setItem('tempToken', response.data.tempToken); // Store the token
    ...
    <Background>
      <Grid container spacing={4} justifyContent="center" alignItems="center">
        <Grid item xs={12} md={6}>
          <ContentBox>
            <img src={img} alt="Company Logo" style={{ marginBottom: 20, maxWidth: '80%' }} />
