...
const AccessMgt = () => {
    ...
    const handleRowClick = (member) => {
        ...
        formik.setValues({
            ...
            photoFile: member.photo,  // Keep the photoFile initially null for new upload
        });
    ...
    //             formData.append("photoFile", selectedMember && selectedMember.photo);
    //         } else if (key === 'modules') {
    ...
    const handleUpdateMember = async () => {
        ...
        formData.append("mobileno", selectedMember.mobileno || "");
        formData.append("whatsappno", selectedMember.whatsappno || "");
        formData.append("designation", selectedMember.designation || "");
        formData.append("status", selectedMember.status || "");
        formData.append("modules", JSON.stringify(formik.values.modules)); // Convert modules array to JSON string

        // Handle the image file separately
        if (formik.values.photoFile) {
            formData.append("photo", formik.values.photoFile); // Use formik's image value if a new file is uploaded
        } else if (selectedMember.photo) {
            formData.append("photo", selectedMember.photo); // Retain existing photo if no new upload
        }
        ...
        <Box sx={{ paddingLeft: 2, position: "relative" }}>
            ...
            <TableContainer component={Paper} sx={{ marginTop: 4 }}>
                <Table>
                    ...
                    <TableBody>
                        {members && members
                            .filter((member) => member.name && member.name.toLowerCase().includes(searchQuery.toLowerCase()))
                            .map((member) => (
                        ...
                        <>
                            <Grid container spacing={2}>
                                <Grid item xs={6} sx={{ paddingRight: '16px' }}>
                                    ...
                                    <TextField label="Whatsapp Number" fullWidth variant="outlined" sx={{ marginBottom: 2 }} value={selectedMember.whatsappno} onChange={(e) => setSelectedMember({ ...selectedMember, whatsappno: e.target.value })} />
